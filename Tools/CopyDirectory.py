...
if __name__ == '__main__':
    ...
    folder_to_zip = r"\\192.168.10.16\VM_Projects\Apache24\htdocs\Web3DAO"
    zipFilename = '%s%s%s' %(r'\\192.168.10.16\VM_Projects\Apache24\htdocs\Web3DAO',time.strftime("_%y%m%d",time.localtime()),'.zip')
    zipFolder( folder_to_zip, zipFilename )

    srcFilename = r"\\192.168.10.16\VM_Projects\Apache24\htdocs\Web3DAO" + time.strftime("_%y%m%d",time.localtime()) + '.zip'
    desFilename = r'C:\Users\chenlei\Desktop\Temp'

    cmd = [
        r'F:\_Projects\Python\PycharmProjects\Tools\CopyDirectory.py',
        srcFilename,
        desFilename
    ]
    main(cmd)
